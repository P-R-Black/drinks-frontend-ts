...
export const BuildDrink = () => {
    ...
    const getAllAlcohols = () => {
        let alcohols = AllDrinksApiData.drinks.map((dr: { base_alcohol: string[]; }) => dr.base_alcohol)
        let filteredAlcohol = alcohols.filter((alc: string) => alc)
    ...
    const getAllIngredients = () => {
        let ingredients = AllDrinksApiData.drinks.map((dr: { ingredients: any; }) => dr.ingredients)
        let filteredIngredients = ingredients.filter((ing: any) => ing)
    ...
    const filterDrinks = (drinks: any[], selectedBaseAlcohols: any[], selectedIngredients: any[]) => {
        return AllDrinksApiData.drinks.filter((drink: { base_alcohol: string | any[]; ingredients: string[]; }) => {
    ...
    const filteredDrinkNames = filterDrinks(AllDrinksApiData.drinks, selectedBaseAlcohols, selectedIngredients);
    ...
    filteredDrinkNames.forEach((fdn: any) => {
        for (let d = 0; d < AllDrinksApiData.drinks.length; d++) {
            if (fdn === AllDrinksApiData.drinks[d].drink_name) {
                filteredDrinksList.push(AllDrinksApiData.drinks[d])
            }
        }
    });
